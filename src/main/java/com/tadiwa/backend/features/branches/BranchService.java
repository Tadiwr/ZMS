...
public class BranchService {
    ...
    public Branch updateBrach(UpdateBranchDTO dto) throws NotFound {
        Optional<Branch> branchOption = branchRepo.findById(dto.getId());
        Optional<PartyDistrict> partyOptional = partyDistService.getPartyDistrictById(dto.getPartyDistrictId());

        if (partyOptional.isEmpty() || branchOption.isEmpty()) {
            throw new NotFound();
        }

        PartyDistrict partyDistrict = partyOptional.get();
        Branch branch = branchOption.get();
